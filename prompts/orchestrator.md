...
1. 时间窗、登录、授权和页面可读性预检。
2. 开始先检查消息；未读筛选为空时检查未筛选的当前会话列表。消息列表异常或普通回复失败只记录局部失败，不重复发送；职位列表和岗位页正常时继续筛选投递。
3. 每个副作用前调用 `RiskGuard`。
4. 消息已处理、无可处理消息或消息步骤局部失败后，继续筛选岗位。
...
出现验证码、身份验证、合同、付费、敏感信息、远程控制、未知下载、账号异常，或职位列表/当前岗位页不可读时立即暂停。仅消息页为空、未读检查失败或普通回复失败不得暂停整轮。
